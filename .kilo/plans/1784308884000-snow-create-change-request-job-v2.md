...
### 1. Read JenkinsJob skill
**File:** `docs/JenkinsJob/AGENTS.md`

Read and understand the Jenkins job conventions:
- Job naming conventions (prefixes: `CI-`, `Tooling-`, `Release-`)
- Job code structure (Imports → Config+Var → Pipeline code → Functional functions → Util functions)
- Code ownership rules (pipeline simplicity, stage length ≤10 lines, validation in parameter definition or DataUtil lib only)
- Shared-libs responsibilities

Apply these rules when creating the Jenkinsfile.

### 2. Create `SnowClient.groovy` in shared lib
...
Follow `createAuditEntry/Jenkinsfile` structure, but apply JenkinsJob skill rules:
- Use appropriate naming prefix if required
- Keep pipeline code simple and show main flow only
- Stages should be ≤10 lines
...
**Stages:**
1. **Create Change Request** — instantiate `SnowClient`, call `createChangeRequest`, store `sys_id` / `number` / `state` in env vars
2. **Output** — echo `CR Number`, `sys_id`, `State`

**Post:** failure/always blocks (same pattern as existing jobs)

## Validation
- Confirm `SnowClient.groovy` compiles (Groovy syntax check)
- Confirm Jenkinsfile matches existing job structure and JenkinsJob skill rules

...
|---|---|---|
| Job granularity | One job per API | User chose this over one job per method. Simpler job management, JOB_FUNCTION routes to method. |
| Job names | `Tooling-SNOW`, `Tooling-ICE`, `Tooling-G3` | User chose Tooling- prefix. Matches JenkinsJob naming convention. |
| Job location | `jobs/dev/<job-name>/Jenkinsfile` | Follows existing folder structure. |
| Existing jobs | Delete and replace | User chose to remove old jobs entirely. New jobs supersede them. |
| Model classes | Full input/output models, one file per API | User chose full models over Map-based parsing. One file per API (SnowModels, ICEModels, G3Models) keeps related models together. |
| Return.json helper | Shared `vars/writeReturnJson.groovy` step | User chose shared helper over inline logic. DRY principle across all 3 jobs. |

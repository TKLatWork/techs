...
- **app**: `app/src/main/java/com/techs/app/` (controllers, services, repositories, config, security — **flat structure**)
- **web**: `web/src/` (React components, hooks, services, pages)

---

## Phase 1: Setup — All Modules Feature-Based Restructuring (US0 / P0)
...
### Domain Layer — Interfaces & Domain Services
...
### App Layer — Repository Implementations
...
- [ ] T049 [P] Implement InMemorySessionRepository (ConcurrentHashMap, userId → Set\<token\> index) in `app/src/main/java/com/techs/app/repository/InMemorySessionRepository.java`

### App Layer — Services & Infrastructure
...
### Web Layer — Shared Infrastructure
...
- [ ] T061 Implement ProtectedRoute component (check auth → redirect to /login; check requiredRight → show Access Denied) in `web/src/features/auth/components/ProtectedRoute.tsx`
...
**Checkpoint**: Foundation ready — domain entities, repositories, PasswordEncoder interface, security, data seeding, and shared web infrastructure are complete. Domain AuthService is wired via constructor injection. User story implementation can now begin.
...
- [ ] T063 [P] [US1] Domain unit test for AuthService.login (valid credentials, invalid credentials, blank credentials — mock UserRepository and PasswordEncoder) in `domain/src/test/java/com/techs/domain/userauth/service/AuthServiceTest.java`
...
- [ ] T072 [US1] Verify login flow end-to-end: navigate to `/login` → enter `admin`/`123` → redirect to `/` with greeting; verify effectiveRights cached in AuthContext
...
- [ ] T073 [P] [US2] Domain unit test for AuthService.getCurrentUser (resolve user from session, compute effectiveRights — mock SessionRepository and AuthorizationService) in `domain/src/test/java/com/techs/domain/userauth/service/AuthServiceTest.java`
...
- [ ] T084 [US2] Verify profile flow: login → navigate to `/profile` → verify all fields displayed; logout → navigate to `/profile` → redirect to `/login`
...
## Phase 5: User Story 5 — Self-Registration (Priority: P1)
...
- [ ] T085 [P] [US5] Domain unit test for AuthService.register (unique username, duplicate username, blank fields, Visitor role assignment — mock UserRepository and RoleRepository) in `domain/src/test/java/com/techs/domain/userauth/service/AuthServiceTest.java`
...
- [ ] T094 [US5] Verify registration flow: navigate to `/register` → create account → redirect to `/login` → login with new credentials → verify Visitor role on profile

**Checkpoint**: Self-Registration is fully functional. New users can register, are assigned Visitor role, and can log in.

---

## Phase 6: User Story 3 — Permission-Based Access Control (Priority: P2)
...
- [ ] T096 [P] [US3] API E2E test for UrlAuthorizationFilter (access granted with right, access denied without right — full Spring context) in `app/src/test/java/com/techs/app/security/UrlAuthorizationFilterTest.java`
...
## Phase 7: User Story 4 — Logout (Priority: P2)
...
**Independent Test**: Log in, click logout, verify redirect to `/login`. Attempt to access `/profile`, verify redirect to `/login`. Log in from another browser — that session remains valid.
...
- [ ] T113 [US4] Verify logout flow: login → click logout → redirect to `/login` → access `/profile` → redirect to `/login`
...
## Phase 8: User Story 6 — Permission Management Page (Priority: P2)

**Goal**: Admin users access a management page to configure roles, rights, user permissions, and feature management. Non-admin users see "Access Denied". The Admin role is fully immutable — all modification attempts are blocked.

**Independent Test**: Log in as admin, navigate to `/permissions`, create a role, assign rights, assign role to a user, verify user gains expected access. Log in as non-admin, verify "Access Denied". Attempt to modify Admin role — verify blocked.
...
### Implementation for User Story 6 — Backend (Domain)

- [ ] T118 [US6] Implement role CRUD methods in AuthorizationService (create custom role, update role rights, delete custom role with Visitor reassignment, prevent Admin role modification — return error for any Admin role mutation attempt) in `domain/src/main/java/com/techs/domain/userauth/service/AuthorizationService.java`
...
### Implementation for User Story 6 — Backend (App Controllers)
...
- [ ] T129 [P] [US6] Implement RoleManager component (MUI Table: list roles, create/edit/delete custom roles, assign rights via checkboxes, Admin role displayed as read-only with all rights — no edit/delete controls) in `web/src/features/permissions/components/RoleManager.tsx`
...
- [ ] T135 [US6] Verify management flow: admin creates role → assigns rights → assigns role to user → user gains access; non-admin sees "Access Denied"; Admin role cannot be modified
...
## Phase 9: User Story 7 — Feature Permission Registration (Priority: P2)
...
- [ ] T138 [US7] Implement registerPermissions method in AuthorizationService (create/update rights with FEATURE_REGISTERED source, idempotent upsert by name, auto-approve — immediately available for assignment) in `domain/src/main/java/com/techs/domain/userauth/service/AuthorizationService.java`
...
- [ ] T140 [US7] Verify registration flow: register permission via API → appears in rights catalog on management page → assign to role/user → access control works
...
- **US1 Login (Phase 3)**: Depends on Phase 2 — foundational auth infrastructure
- **US2 Profile (Phase 4)**: Depends on Phase 3 (US1 Login) — requires authentication to be working
- **US5 Registration (Phase 5)**: Depends on Phase 2 — can start in parallel with US1/US2 after Phase 2
- **US3 Access Control (Phase 6)**: Depends on Phase 2 — can start after foundational authorization infrastructure
- **US4 Logout (Phase 7)**: Depends on Phase 3 (US1 Login) — requires session management from login
- **US6 Permission Mgmt (Phase 8)**: Depends on Phase 6 (US3 Access Control) — requires authorization infrastructure
- **US7 Registration API (Phase 9)**: Depends on Phase 8 (US6 Permission Mgmt) — extends permission management
- **Polish (Phase 10)**: Depends on all user stories being complete

### User Story Dependencies

```
Phase 1 (Setup) → Phase 2 (Foundational) → ┬─ US1 (Login) ──→ US2 (Profile)
                                            │               └──→ US4 (Logout)
                                            ├─ US5 (Registration) [parallel with US1]
                                            └─ US3 (Access Control) ──→ US6 (Perm Mgmt) ──→ US7 (Reg API)
```
...
- Tests MUST be written and FAIL before implementation (three layers: domain unit → API E2E → web E2E)
...
## Parallel Example: User Story 1 (Login) — Three-Layer Tests

```
# Launch all three test layers together (they target different files/modules):
Task T063: Domain unit test — AuthServiceTest in domain/src/test/
Task T064: API E2E test — AuthControllerTest in app/src/test/
Task T065: Web component test — LoginForm.test.tsx in web/src/features/auth/__tests__/
...
## Parallel Example: User Story 2 (Profile) — Three-Layer Tests

```
# Launch all three test layers together:
Task T073: Domain unit test — AuthServiceTest.getCurrentUser in domain/src/test/
Task T074: API E2E test — UserControllerTest in app/src/test/
Task T075: Web component test — ProfileCard.test.tsx in web/src/features/profile/__tests__/
```
...
5. Deploy/demo if ready — proves the full web → api → domain dependency chain

### Incremental Delivery

1. Complete Setup + Foundational → Foundation ready
2. Add US1 (Login) → Test independently → **MVP!**
3. Add US2 (Profile) + US5 (Registration) → Test independently → Multiple user types work
4. Add US3 (Access Control) + US4 (Logout) → Test independently → Security layer complete
5. Add US6 (Permission Mgmt) → Test independently → Admin can configure permissions
6. Add US7 (Registration API) → Test independently → System is extensible
...
- Developer A: US1 (Login) → US2 (Profile) → US4 (Logout)
- Developer B: US5 (Registration) → US3 (Access Control)
- Developer C: US6 (Permission Mgmt) → US7 (Registration API)
...
- Three-layer testing: domain unit (JUnit 5, no Spring) → API E2E (Spring Boot Test) → web E2E (Vitest + @testing-library/react)
...
- Domain AuthService is the full orchestrator — receives all dependencies via constructor injection
- Multiple concurrent sessions per user are allowed; logout invalidates only the current session
- Admin role is fully immutable — management page blocks all modification attempts
